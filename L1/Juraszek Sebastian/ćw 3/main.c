...
#define RED_LED BIT6                        // Czerwona LED -> P1.6
#define GREEN_LED BIT0                      // Zielona LED -> P1.0
...
void main(void){
   ...
   // Ustawianie diod i przycisku
    ...
    P1IES &= ~SW;                           // Puszczenie przycisku jako wyzwolenie przerwania
    P1IE |= SW;                             // Uruchomienie przerwania na przycisku

    TA0CCTL0 |= CCIE;                       // Ustawienie wywolania przerwania timera 0 po odliczeniu ustawionego czasu - timer 0
    TA0CTL |= TASSEL_2  + TACLR + ID_3;     // Ustawienie liczenia - "w gore", Zegar -> ACLK, Zerowanie stanu czasu - timer 0
    TA1CCTL0 |= CCIE;                       // Ustawienie wywolania przerwania timera 1 po odliczeniu ustawionego czasu - timer 1
    TA1CTL |= TASSEL_2  + TACLR + ID_3;     // Ustawienie liczenia - "w gore", Zegar -> ACLK, Zerowanie stanu czasu - timer 1

    TA0CCR0 = 50000;                        // Ustawienie Timera 0 na 5 sekundy
    TA1CCR0 = 25000;                        // Ustawienie Timera 1 na 2 sekunde
...
__interrupt void CCR1_ISR(void){
    ...
    if (takt==10){
        for(y=0; y<6; y++){                            // wykonanie 6 zmian stanu diody czerownej
...
__interrupt void CCR0_ISR(void){
    ...
    if(takt==10){                             // wykonanie kodu po zliczeniu 10 przerwan
        ...
        else if(licznik==5)
        {
            ...
            P1OUT &= ~RED_LED;                  // wylczenie led czerwonej
            takt=0;
        }
