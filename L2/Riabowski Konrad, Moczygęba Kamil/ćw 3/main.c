...
//DEFINICJA ZMIENNYCH CALKOWITYCH DO REALIZACJI PROGRAMU
...
void main(void)
{
    ...
    //DIODA ZIELONA
    P1DITR  |=  ZIELONA;        //USTAWIENIE NA WYJSCIU BITU DIODY
    P1OUT   &=  ~ZIELONA;       //WYLACZENIE DIODY ZIELONEJ
    //DIODA CZERWONA
    P1DIR   |=  CZERWONA        //USTAWIENIE NA WYJSCIU BITU DIODY
    P1DIR   &=  ~CZERWONA       //WYLACZENIE DIODY CZERWONEJ
    //PRZELACZNIK
    ...
    TA0CTL   |=  TASSEL_2 + TACLR + ID_3;   //WYKONYWANIE ZLICZANIA NA TIMERZE SMCLK
    ...
    _bis_SR_register(LMP0_bits + GIE);
}
...
//NASTEPUJE PRZERWANIE PO ODLICZANIU TIMERA0
...
__interrupt void CCR0_ISR(void)
{
    ...
    if (x10==10)                //JESLI ZMIENNA X10 BEDZIE WYNOSILA 10
    {
        ...
        else                    //W POZOSTALYCH PRZYPADKACH
        {
            ...
            TA0CTL  &= ~MC_3        //ZATRZYMANIE SIE TIMERA0
            TA1CTL  |= MC_1         //URUCHOMIENIE SIE TIMERA1
            P1OUT   &= ~ZIELONA;    //WYLACZENIE DIODY ZIELONEJ
            P1OUT   &= ~CZERWONA;   //WYLACZENIE DIODY CZERWONEJ
            x10 = 10;               //WYZEROWANIE ZMIENNEJ X10
        }
        x10 = 0;                    //WYZEROWANIE ZMIENNEJ X10
        counter++;                  //INKREMENTACJA ZMIENNEJ COUNTER
    }
}

//WYKONANIE PRZERWANIA  ORAZ ODLICZANIE TIMERA1
...
__interrupt void CCR1_ISR(void)
{
    while(counter <= 4)                 //WYKONANIE 2 MIGNIEC DIODY CZERWONEJ
    {
        ...
        __delay_cycles(1000000)         //OPOZNIENIE WYKONANIE PENTLI - 1S
